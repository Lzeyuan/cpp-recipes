...
// 泛型类
template <typename Signature> class DelegateThreadSafe;

// 特化SafeSignal，为了符合函数直觉：SafeSignal<int(int,int)>
// RET仅为了模板匹配，实际忽略返回值
template <typename RET, typename... ARGS> class DelegateThreadSafe<RET(ARGS...)> {
public:
  using Callback = std::function<void(ARGS...)>;
  using PublisherImpl = detail::PublisherImpl<Callback>;
  using SubscriberToken = detail::SubscriberToken<Callback>;

  DelegateThreadSafe() : publisher_impl_(std::make_shared<PublisherImpl>()) {}
